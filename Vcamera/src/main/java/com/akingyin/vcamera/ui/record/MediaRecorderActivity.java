...
import com.akingyin.vcamera.ui.record.views.ProgressView;
import com.akingyin.vcamera.ui.record.views.ThemeRadioButton;
import com.akingyin.vcamera.utils.ConvertToUtils;
import com.akingyin.vcamera.utils.NetworkUtils;
import java.io.File;
...
public class MediaRecorderActivity extends BaseActivity
	...
	private void initMediaRecorder() {
		...
		mMediaRecorder.setVideoBitRate(
	...
	public void onClick(View v) {
		...
		if (i == R.id.record_import) {
			...
			if (mMediaObject != null) {
				...
				if (mProgressView != null) mProgressView.invalidate();

				//检测按钮状态
				checkStatus();
			}
		} else if (i == R.id.record_filter) {
			if (mRecordFilter.isChecked()) {
				mRecordFilterLayout.setVisibility(View.GONE);
				mRecordFilter.setChecked(false);
			} else {
			...
			startActivityForResult(
					Intent.createChooser(intent, getString(R.string.record_camera_import_image_choose)),
					REQUEST_CODE_IMPORT_IMAGE);
		} else if (i == R.id.import_video) {
			if (mPopupWindow != null) {
				mPopupWindow.dismiss();
				mPopupWindow = null;
			}
			...
			startActivityForResult(
					Intent.createChooser(intent, getString(R.string.record_camera_import_video_choose)),
	...
	private void loadFilter() {
		if (!isFinishing() && mRecordFilterContainer.getChildCount() == 0) {
			...
			for (int i = 0; i < FILTER_ICONS.length; i++) {
				ThemeRadioButton
						filterView = (ThemeRadioButton) mInflater.inflate(R.layout.view_radio_item, null);
